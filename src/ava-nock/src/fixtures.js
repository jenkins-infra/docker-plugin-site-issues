...
import {decodeResponse} from './encoding.js';
...
function getFixturePath(ava) {
  const testPath = fileURLToPath(ava.meta.file);
  ...
  if (config.fixtureDir) {
    ...
    fixtureDir = fileURLToPath(ava.meta.snapshotDirectory);
  } else {
...
export async function readFixture(filename) {
  ...
  return parseJSON(await fs.readFile(filename, 'utf8'))
}
...
export async function saveFixture(t) {
  ...
  if (
    ...
    await writeFixture(fixturePath, outputData);
  }
}
